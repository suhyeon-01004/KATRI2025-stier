...
**이수현 담당: 정적장애물 회피.** 아래 알고리즘 설명은 제공된 `src_251031-1725.zip`의 코드 분석에 근거합니다. 팀 전체 워크스페이스이므로 모든 패키지를 개인 구현으로 표시하지 않으며, 파일별 작성자·최종 실차 실행 조합·정량 성능은 이 스냅샷만으로 확정하지 않습니다.
