...
def brake_di_decision(di): 
    ...
    Kp_brake = 15       # 최적화 parameter
    Kp_brake_min = 10   # 최적화 parameter
    
    brake = speed_error * Kp_brake
    min_brake = Kp_brake_min * speed_error
...
def publish_Ctl(di, v, brake):
    ...
    print("Publish_msg /lidarDeg:", steer_msg.data)

    vel_msg = Float32()
    vel_msg.data = v
    vel_pub.publish(vel_msg)
    print("Publish_msg /lidarKPH:", vel_msg.data)

    brake_msg = Float32()
    brake_msg.data = brake
    brake_pub.publish(brake_msg)
    print("Publish_msg /lidarBrake:", brake_msg.data)
...
if __name__ == '__main__':
    ...
    rospy.Subscriber("/stop_sign", Int32, stop_sign_callback)
    rospy.Subscriber("/current_velocity", Float32, feedback_callback)


    vel_pub = rospy.Publisher("/missionKPH", Float32, queue_size=1)
    steer_pub = rospy.Publisher("/missionDeg", Float32, queue_size=1)
    brake_pub = rospy.Publisher("/lidarBrake", Float32, queue_size=1)
