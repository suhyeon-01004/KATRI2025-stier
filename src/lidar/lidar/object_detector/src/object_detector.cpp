...
// =====================[ ADD: 섹션3 부채꼴 필터 함수 ]=====================
static pcl::PointCloud<PointT>::Ptr sectorFilterDeg(
    const pcl::PointCloud<PointT>::Ptr& in, double radius_m, double half_deg)
{
    const double PI = 3.14159265358979323846;
    const double half = half_deg * (PI / 180.0);
    pcl::PointCloud<PointT>::Ptr out(new pcl::PointCloud<PointT>);
    out->reserve(in->size());
    for (const auto& p : in->points) {
        const double r = std::hypot(p.x, p.y);
        if (r > radius_m) continue;
        const double th = std::atan2(p.y, p.x); // 진행방향=0 rad
        if (std::fabs(th) <= half) out->push_back(p);
    }
...
void sectionCallback(const std_msgs::Int32::ConstPtr& sec_msgs) {
    ...
    // =========================[ ADD: 섹션 3 전용 파라미터 ]========================
        ...
        // ASCII 경고 + 스팸 방지
        ROS_WARN_THROTTLE(1.0, "### SECTION 3 ROI ACTIVE (sector r=2.0m, half=57deg) ###");

        // PassThrough는 넉넉히, 실제 부채꼴은 ROI()에서 자름
        xMin = 0.0;  xMax = 10.0;
        yMin = -5.0; yMax = 5.0;
        zMin = -0.4; zMax = 1.2;
...
pcl::PointCloud<PointT>::Ptr ROI (const sensor_msgs::PointCloud2ConstPtr& input) {
    ...
    // =====================[ ADD: 섹션3 부채꼴 최종 마스킹 ]=====================
    if (section == 3) {
        cloud_filtered = sectorFilterDeg(cloud_filtered, 5.0, 57.0);
    }
    // ========================================================================
...
void cluster(pcl::PointCloud<PointT>::Ptr input) {
    ...
    //Eigen::Matrix3f rvec;
    //rvec << -0.01093675, -0.99993183, -0.00408974,
    //        -0.398308  ,  0.00810792, -0.9172159,
    //         0.91718653, -0.00840239, -0.39836952;
    //Eigen::Vector3f tvec( -0.01477849, 0.32916884, 0.81618255);

    //Eigen::Matrix3f cameraMatrix;
    //cameraMatrix << 916.509137,     0.,         660.08973047,
