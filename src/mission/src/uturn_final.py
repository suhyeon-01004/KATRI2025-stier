...
MAX_AREA_THRESHOLD = 0.5
MAX_LENGTH_THRESHOLD = 0.5
Z_CENTER_THRESHOLD = 0
...
class ObjectMarkerVisualizer:
    def __init__(self):
        rospy.init_node('uturn_final', anonymous=True)
        rospy.Subscriber('/object_info', ObjectInfo, self.callback)
        rospy.Subscriber('/section', Int32, self.section_callback)
        rospy.Subscriber('/lidarDeg', Float32, self.lidar_deg_callback)
        rospy.Subscriber('/ublox_position_receiver/navpvt', NavPVT, self.nav_callback)
        ...
        self.deg_pub = rospy.Publisher('/missionDeg', Float32, queue_size=1)
        self.ld_pub = rospy.Publisher('/lidar_pp_ld', Int32, queue_size=1)
        self.mission_lane_control_pub = rospy.Publisher('/mission_lane_control', Int32, queue_size=1, latch=True)
        # self.avoid_finish_pub = rospy.Publisher('/avoid_finish', Int32, queue_size=1)
        self.mission_finish_pub = rospy.Publisher('/mission_finish', Int32, queue_size=1)
        if SHOW_ROI_MARKER:
        ...
        self.lane_control_active = False
        self.control_enabled = False
        self.finish_sent = False
        self.steering_angle = 0.0
        self.heading_kalman = KalmanFilter(0.1, 0.1, 0.1, 0.0)
    ...
    def callback(self, msg):
        ...
        if len(centers) >= 2:
            ...
            if len(groups) == 2:
                ...
                spline_marker, path_msg = self.draw_spline_curve(midpoints)
                if spline_marker:
        ...
        path_valid = False
        if path_msg and path_msg.cnt > 0:
            for px, py in zip(path_msg.x, path_msg.y):
                if abs(px) > 1e-3 or abs(py) > 1e-3:
                    path_valid = True
                    break

        if self.lane_control_active and not self.control_enabled and (roi_detected and path_valid):
            self.control_enabled = True
            self.lane_control_active = False
            self.mission_lane_control_pub.publish(0)
            rospy.loginfo("Lane control disabled, switching to uturn controller")

        if self.control_enabled:
            if self.finish_sent:
                return
            if path_msg and published_path:
                self.path_pub.publish(path_msg)
            else:
                self.publish_empty_path()
            self.vel_pub.publish(Float32(BASE_SPEED_KPH))
            self.deg_pub.publish(Float32(self.steering_angle))
            self.ld_pub.publish(Int32(1.5))
    ...
    def section_callback(self, msg):
        ...
        if msg.data == 8:
            ...
            self.lane_control_active = True
            self.mission_lane_control_pub.publish(1)
            rospy.loginfo("Section 8 detected, enabling lane control node")

    def lidar_deg_callback(self, msg):
        self.steering_angle = msg.data
        if (self.steering_angle == 0) and self.control_enabled:
    ...
    def nav_callback(self, msg):
        ...
        heading_diff = angle_difference(self.prev_heading, raw_heading)
        filtered_heading = self.heading_kalman.update(heading_diff)
        new_heading = (self.prev_heading + filtered_heading) % 360
        self.prev_heading = new_heading
        self.current_heading_deg = new_heading

        if self.control_enabled and not self.finish_sent and self.current_heading_deg is not None:
            heading_cog = self.current_heading_deg  # 0~360, 북=0°, 시계+
            rospy.loginfo(f"[uturn_final] heading_cog={heading_cog:.2f}")
            if 95.0 < heading_cog < 140.0:  # TODO: adjust bounds to desired COG range
                # self.avoid_finish_pub.publish(Int32(1))
                self.mission_finish_pub.publish(Int32(8))
                self.finish_sent = True
                rospy.loginfo("U-turn complete, published avoid_finish")
