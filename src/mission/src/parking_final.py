...
if __name__ == '__main__':
    rospy.init_node('parking_final', anonymous=True)

    # ROS Publisher
    start_park_pub = rospy.Publisher('/start_park_final', Bool, queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        if start_obstacle is not None and start_obstacle[0] < 0.8:
            while not rospy.is_shutdown():
