...
def calculate_cone_closed():
    ...
    while not rospy.is_shutdown():
        image_msg = rospy.wait_for_message("/usb_cam1/image_raw", Image) # 시뮬용 09.08 주석
...
if __name__ == '__main__':
    rospy.init_node('parking_semi', anonymous=True)

    # ROS Publisher
    start_park_pub = rospy.Publisher('/start_park_semi', Bool, queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        if start_obstacle is not None and start_obstacle[0] < 1.5: # 목표 장애물과의 거리
            while not rospy.is_shutdown():
