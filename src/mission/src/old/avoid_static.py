...
from std_msgs.msg import Float32, Header, ColorRGBA, String, Int32
...
class VelodyneFilterAndLanes:
    def __init__(self):
        ...
        self.left_offset_m  = float(gp("~left_offset_m",  6.7))
        self.right_offset_m = float(gp("~right_offset_m", 3.0))
        # self.left_offset_m  = float(gp("~left_offset_m",  5.3))
        # self.right_offset_m = float(gp("~right_offset_m", 1.8))
...
class SimplePathPlanner:
    ...
    def __init__(self):
        ...
        self.back_look_s     = float(gp("~back_look_s",   2.0))  # ★ 약간 후방 허용
        ...
        self.two_side_extra_margin = float(gp("~two_side_extra_margin", 0.10))
        ...
        self.kph_default = float(gp("~default_kph", 7.0)) 
        self.kph_decel   = float(gp("~decel_kph",   4.0)) 
        self.initial_deg = float(gp("~initial_deg", 0.0)) 
        self.ld_value    = int(gp("~ld_value",      7)) 
        self.section_id = int(rospy.get_param("~section_id", 9)) 

        # 현재 상태값 초기화
        self.lidar_kph = self.kph_default
        self.lidar_deg = self.initial_deg
        # --- state
        self.section = -1
        self.seen_section9_once = False    # 섹션9 처음 진입 시 lane ON 1회 보장용
        self.static_active = False         # static 모드 ON/OFF
        self.landmark_latched = False      # 로드마크 트리거 래치
        self.dynamic_latched  = False      # 동적 트리거 래치

        ##############################################################################
        # 토픽
        self.pub_path_mk    = rospy.Publisher("/path_markers", Marker, queue_size=10, latch=True)
        self.pub_nav_path   = rospy.Publisher("/nav_path", Path, queue_size=1)
        ...
        self.pub_lane_ctrl = rospy.Publisher("/mission_lane_control", Int32, queue_size=10, latch=True)
        self.sub_section   = rospy.Subscriber("/section", Int32, self.cb_section, queue_size=10)
        self.sub_landmark  = rospy.Subscriber("/tunnel_landmark", Int32, self.cb_landmark, queue_size=10)
        self.sub_lidar_deg = rospy.Subscriber("/lidarDeg", Float32, self.cb_lidar_deg, queue_size=10) 
        self.sub_stop = rospy.Subscriber("/moving_stop_cmd", Int32, self.cb_move_cmd, queue_size=1) 
        self.sub_force_lane = rospy.Subscriber("/static_force_lane", Int32, self.cb_force_lane, queue_size=1) # 강제로 lane ON 테스트용
        ...
        # self.path_offset_y = -0.0 #float(gp("~path_offset_y", -5.0)) 
    ...
    def _inflate_and_filter_boxes(self, boxes_xy, v, n, o, left_seg):
        """
        반환: list of (s_min, s_max, d_min, d_max, a, b)
        (s_min..d_max)은 '팽창 외접 직사각형' 범위, a=장축/ b=단축(좌우) 반경
        """
        # 타원 반경 (차량 + 여유 + 장애물 마진)
    ...
    def _ellipse_delta_d_at_s(s, s_min, s_max, a, b):
        # core: [s_min+a, s_max-a] -> dd=b, 그 외 캡에서 dd=b*sqrt(1 - (ds/a)^2)
        s_core_min = s_min + a
        s_core_max = s_max - a
    ...
    def _build_path_points(self, right_line, left_line, boxes_xy):
        ...
        for s in ss:
            ...
            # 현재 s에 영향을 주는 금지 밴드 수집
            relevant = []
            for box in boxes_sd:
                if not (box[0] <= s <= box[1]):  # s_min..s_max
                    continue
                dm, dx = self._forbidden_band_at_s(s, box)
            ...
            d_des = a_band + clamp(alpha_use, 0.0 + self.alpha_eps_inside_band, 1.0 - self.alpha_eps_inside_band)*(b_band - a_band)
            if prev_d is not None:
        ...
        if path_sd:
            ...
            if used_left and not used_right and (left_front_s is not None):
                ...
                if hold_s > path_sd[-1][0] + 1e-3: path_sd.append((float(hold_s), float(d_hold)))
    ...
    def _publish_path(self, pts_xy):
        ...
        for (x,y) in pts_xy:
            ...
            ps.pose.position.x = float(x); ps.pose.position.y = float(y); ps.pose.position.z = 0.0
            ps.pose.orientation.w = 1.0; path.poses.append(ps)
        self.pub_nav_path.publish(path)

        # PathInfo: 호길이 균일 리샘플(20개)
    ...
    ############################### 시뮬 추가 09/13 ###############################
        ...
        self.pub_ld.publish(Int32(self.ld_value)) 

        # 최종 명령: 속도 0이면 조향 0, 아니면 /lidarDeg
        kph_cmd = float(self.lidar_kph) 
        deg_cmd = 0.0 if kph_cmd <= 0.0 else float(self.lidar_deg) 
        self.pub_mission_kph.publish(Float32(kph_cmd)) 
        self.pub_mission_deg.publish(Float32(deg_cmd))

    def _want_static(self) -> bool:
        # 섹션9 & (로드마크 또는 동적) 이면 static ON 희망
        return (self.section == self.section_id) and (self.landmark_latched or self.dynamic_latched)

    def _try_switch_to_static(self, reason: str):
        if self.section != self.section_id:
            return
        if not self.static_active and self._want_static():
            self.static_active = True
            self.pub_lane_ctrl.publish(Int32(0))  # lane OFF → static ON
            rospy.loginfo("[static] switch -> static ON (reason=%s)", reason)

    def switch_to_lane(self):
        if self.static_active:
            self.static_active = False
            self.pub_lane_ctrl.publish(Int32(1))  # lane ON
            rospy.loginfo("[static] switch -> lane ON")
        # 필요하면 래치 해제(선택)
        # self.landmark_latched = False
        # self.dynamic_latched  = False

    def _can_pub_cmd(self) -> bool:
        # 이 노드가 KPH/DEG를 발행할지 여부
        return (self.section == self.section_id) and self.static_active
    ...
    def cb_lanes(self, msg: MarkerArray):
        ...
        ############################### 시뮬 추가 09/13 ###############################
        # [중요] 섹션 9 아니거나 static OFF면 리턴
        if self.section != self.section_id or not self.static_active:
            return
        ##############################################################################
        right = left = None
        ...
        if right is None or left is None:
            msg_empty = PathInfo(); msg_empty.cnt = 0
            msg_empty.x = [0.0]*PATH_NUM_POINTS; msg_empty.y = [0.0]*PATH_NUM_POINTS
            self.pub_local_path.publish(msg_empty); return
    ...
    def cb_lidar_deg(self, msg: Float32): # 시뮬 추가 09/13
        self.lidar_deg = float(msg.data) # 시뮬 추가 09/13

    def cb_section(self, msg: Int32):
        self.section = int(msg.data)
        # 섹션 9 처음 들어오면 lane ON을 1회 보장
        if self.section == self.section_id and not self.seen_section9_once:
            self.seen_section9_once = True
            self.static_active = False
            self.pub_lane_ctrl.publish(Int32(1))  # lane ON
            rospy.loginfo("[static] enter section %d -> lane ON (bootstrap)", self.section_id)

    def cb_landmark(self, msg: Int32):
        # 외부 로드마크: 1이면 래치
        if int(msg.data) == 1 and self.section == self.section_id:
            self.landmark_latched = True
            self._try_switch_to_static(reason="landmark")

    def cb_move_cmd(self, msg: Int32):
        mc = int(msg.data)

        # 동적 트리거 래치 (권장: 정지 시점만)
        if mc == 1 and self.section == self.section_id:
            self.dynamic_latched = True
            self._try_switch_to_static(reason="dynamic_stop")

        # 속도 상태 갱신
        if   mc == 1: self.lidar_kph = self.kph_decel
        elif mc == 2: self.lidar_kph = 0.0
        elif mc == 3: self.lidar_kph = self.kph_default
        else:
            rospy.logwarn("unknown move_cmd=%s", mc)
            return

        # static 모드이고 섹션9일 때만 KPH/DEG 직접 발행
        if self._can_pub_cmd():
            kph_cmd = float(self.lidar_kph)
            deg_cmd = 0.0 if kph_cmd <= 0.0 else float(self.lidar_deg)
            self.pub_mission_kph.publish(Float32(kph_cmd))
            self.pub_mission_deg.publish(Float32(deg_cmd))

    def cb_force_lane(self, msg: Int32): # 강제로 lane ON 테스트용
    # data==1 이면 강제 lane 전환
        if int(msg.data) == 1:
            rospy.logwarn("[static] force lane requested via /static_force_lane")
            self.switch_to_lane()
    ##############################################################################
...
def main():
    ...
    rospy.loginfo("Fusion node ready. Publishes /local_path(PathInfo,N=20) + /nav_path(Path) + /path_markers + /inflated_obstacles. Fixed frame='%s'", FIXED_FRAME)
    rospy.spin()
