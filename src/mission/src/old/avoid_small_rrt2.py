...
AVOID_FINISH_TIME = 3  # 회피 종료를 위한 대기 시간 설정
...
class RRTStar:
    def __init__(self, start, goal, dist_to_connect=0.5, num_of_cand_node=7, path_len=20, arrive_dis=0.3, angle=28):
        self.start = start  # Node 객체
        self.goal = goal  # Node 객체
...
def callback_ob(msg):
    ...
    for i in range(msg.objectCounts):
        ...
        # 장애물을 앞뒤로 연장
        pt1 = (cx[i] - lenx[i] / 2 - 1.2, cy[i])
        obstacle_list.append(pt1)
        pt2 = (cx[i] - lenx[i] / 2 - 1.2, cy[i] + leny[i] / 4 + 0.4)
        obstacle_list.append(pt2)
        pt3 = (cx[i] - lenx[i] / 2 - 1.2, cy[i] - leny[i] / 4 - 0.4)
        obstacle_list.append(pt3)
        pt4 = (cx[i] - lenx[i] / 2 - 0.6, cy[i] - leny[i] / 2 - 0.5)
        obstacle_list.append(pt4)
        pt5 = (cx[i] - lenx[i] / 2 - 0.6, cy[i] + leny[i] / 2 + 0.5)
        obstacle_list.append(pt5)

    obstacles = obstacle_list  # 글로벌 변수 업데이트
    ...
    while not rospy.is_shutdown():
        ...
        if started:
            if current_obstacle_list:
                print('장애물이 감지되었습니다. 회피 주행을 시작합니다.')
                started = False
                obstacle_closed = True  # 장애물이 감지됨을 표시
                # continue를 제거하여 RRT* 실행
            else:
                print('장애물이 감지되지 않았습니다.')

            # continue를 제거하여 아래의 RRT* 코드가 실행되도록 함

        if finished:
            if start_time is None:
                start_time = time.time()
            time_elapsed = time.time() - start_time
            if 0 <= time_elapsed < AVOID_FINISH_TIME:
            ...
            elif time_elapsed >= AVOID_FINISH_TIME:
                print('회피 주행을 종료합니다.')
                if args.tunnel:
        ...
        if path is not None:
            ...
            print('RRT* 알고리즘으로 유효한 경로를 찾지 못했습니다.')

        # 장애물이 더 이상 감지되지 않으면 회피 주행 종료
        if obstacle_closed and not finished:
            if not current_obstacle_list:
                print('장애물을 통과했습니다. 회피 주행을 종료합니다.')
                finished = True
                start_time = time.time()
