...
float calculate_curvature(float x1, float y1, float x2, float y2, float x3, float y3) {
    ...
    float s = (a + b + c) / 2;
    float area = sqrt(std::max(0.0f, s * (s - a) * (s - b) * (s - c)));
    return (a*b*c > 1e-6f) ? (4 * area / (a * b * c)) : 0.0f;
}
...
float calculate_adaptive_Ld(float speed_kph, float curvature) {
    ...
    float v = speed_kph * (1000.0f / 3600.0f); // m/s
    float ld_speed = base_ld + k_speed * powf(v, 0.7f);
    float ld_curvature = std::max(0.0f, 1.0f - k_curvature * curvature);
    float ld = ld_speed * ld_curvature;
    return std::clamp(ld, min_ld, max_ld);
}

float adaptive_lookahead()
{
    // 과거 방식: 속도와 경로 곡률 기반 Ld 계산
    const int cnt = rddf.getCount();
    const int cur_idx = car.idx_current;
    const int first_idx  = std::min(cnt - 1, std::max(0, cur_idx + 10));
    const int second_idx = std::min(cnt - 1, std::max(0, cur_idx + 20));

    const float curv = calculate_curvature(
        rddf.getX(cur_idx),  rddf.getY(cur_idx),
        rddf.getX(first_idx),  rddf.getY(first_idx),
        rddf.getX(second_idx), rddf.getY(second_idx)
    );

    float Ld = calculate_adaptive_Ld(car.gps_speed, curv);
    return Ld;
}
...
static float predict_dt = 0.15f; // [s] 예측 시간 창

// ---- [NEW] 예측 상태 계산 (CarState 사용) ----
CarState predict_future_state(double current_x,
                              double current_y,
                              ...
                              float  dt)
{
    CarState result = car; // copy current state

    const float L = car.wheelbase;
    const float v = std::max(0.0f, current_speed_kph) * (1000.0f / 3600.0f);
    const float steer_rad = current_steer_deg * (PI_ / 180.0f);
    const float beta = atan(0.5f * tan(steer_rad));

    const float k1_x       = v * cos(current_heading_rad + beta);
    const float k1_y       = v * sin(current_heading_rad + beta);
    const float k1_heading = (v * cos(beta) * tan(steer_rad)) / std::max(1e-6f, L);

    const float k2_x       = v * cos(current_heading_rad + beta + 0.5f * dt * k1_heading);
    const float k2_y       = v * sin(current_heading_rad + beta + 0.5f * dt * k1_heading);
    const float k2_heading = (v * cos(beta) * tan(steer_rad)) / std::max(1e-6f, L);

    const float k3_x       = v * cos(current_heading_rad + beta + 0.5f * dt * k2_heading);
    const float k3_y       = v * sin(current_heading_rad + beta + 0.5f * dt * k2_heading);
    const float k3_heading = (v * cos(beta) * tan(steer_rad)) / std::max(1e-6f, L);

    const float k4_x       = v * cos(current_heading_rad + beta + dt * k3_heading);
    const float k4_y       = v * sin(current_heading_rad + beta + dt * k3_heading);
    const float k4_heading = (v * cos(beta) * tan(steer_rad)) / std::max(1e-6f, L);

    result.x       = current_x + (dt / 6.0f) * (k1_x + 2.0f * k2_x + 2.0f * k3_x + k4_x);
    result.y       = current_y + (dt / 6.0f) * (k1_y + 2.0f * k2_y + 2.0f * k3_y + k4_y);
    result.heading = current_heading_rad + (dt / 6.0f) * (k1_heading + 2.0f * k2_heading + 2.0f * k3_heading + k4_heading);

    const int idx_max = rddf.getMaxIdx();
    const int search_start = std::max(0, current_idx - 10);
    const int search_end   = std::min(idx_max, current_idx + 50);

    float min_dist = std::numeric_limits<float>::max();
    int   best_idx = current_idx;
    for (int i = search_start; i <= search_end; ++i) {
        const double dx = rddf.getX(i) - result.x;
        const double dy = rddf.getY(i) - result.y;
        const float d   = std::sqrt(dx*dx + dy*dy);
        if (d < min_dist) { min_dist = d; best_idx = i; }
    }
    result.idx_current = best_idx;
    return result;
}
...
// ---- [PP with future prediction blend] ----
const float steer_now = calculate_steer_using_pure_pursuit(car.x, car.y, car.heading);

float steer_pred = steer_now;
const float use_pred_kph_thresh = 3.0f;
const float pred_full_kph       = 25.0f;
if (car.gps_speed >= use_pred_kph_thresh) {
    const CarState ps = predict_future_state(
        car.x, car.y, car.gps_speed, car.heading, car.idx_current, (float)car.feedback_steer, predict_dt
    );
    steer_pred = calculate_steer_using_pure_pursuit(ps.x, ps.y, ps.heading);
}
float t = 0.0f;
if      (car.gps_speed <= use_pred_kph_thresh) t = 0.0f;
else if (car.gps_speed >= pred_full_kph)       t = 1.0f;
else                                           t = (car.gps_speed - use_pred_kph_thresh) / (pred_full_kph - use_pred_kph_thresh);
const float steer_final = steer_now + (steer_pred - steer_now) * t;
drive_cmd.Deg = (int)std::lround(steer_final);
// speed control
